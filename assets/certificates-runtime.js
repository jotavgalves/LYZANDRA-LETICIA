(() => {
  const DEFAULT = {
    ...
    images: [
      {
        id: 'certificate-1',
        src: 'https://lycilios.vercel.app/images/certificate.png',
        alt: 'Certificado profissional de conclusão',
        visible: true
      }
    ],
    benefits: {
      ...
      items: [
        ...
        'Mais credibilidade no atendimento',
        'Conclusão com mais confiança'
      ]
    },
    final: {
      visible: true,
  ...
  function normalize(value) {
    ...
    return {
      ...
      benefits: {
        ...benefits,
        visible: benefits.visible !== false,
        items: Array.isArray(benefits.items) ? benefits.items.map(item => String(item || '')).filter(Boolean) : clone(DEFAULT.benefits.items)
      },
      final: {
        ...final,
        visible: final.visible !== false,
        title: String(final.title || DEFAULT.final.title),
        text: String(final.text || DEFAULT.final.text)
      }
    };
  }
  ...
  function ensureStyle() {
    ...
    style.id = 'ly-certificates-style';
    style.textContent = `
      ...
      [data-ly-certificates-media][data-aspect="4/3"]{aspect-ratio:4/3}
      [data-ly-certificates-media][data-aspect="16/9"]{aspect-ratio:16/9}
      [data-ly-certificates-media][data-aspect="1.414/1"]{aspect-ratio:1.414/1}
      ...
      [data-ly-certificates-final] h3{margin:0;font-family:var(--font-serif,Georgia,serif);font-size:clamp(21px,3vw,28px);font-weight:700}
      [data-ly-certificates-final] p{margin:9px auto 0;max-width:650px;color:var(--muted-foreground);font-size:14px;line-height:1.65}
  ...
  function appendHighlightedText(host, text, highlight) {
    ...
    strong.textContent = full.slice(index, index + needle.length);
  ...
  function makeButton(direction, onClick) {
    ...
    button.type = 'button';
    button.dataset.lyCertificatesArrow = direction;
    button.setAttribute('aria-label', direction === 'prev' ? 'Certificado anterior' : 'Próximo certificado');
  ...
  function render(config, section) {
    ...
    const visibleImages = config.images.filter(item => item.visible !== false && item.src);
    const images = visibleImages.length ? visibleImages : clone(DEFAULT.images);
    let index = Math.max(0, Math.min(Number(section.dataset.lyCertificateIndex) || 0, images.length - 1));

    section.innerHTML = '';
    const root = document.createElement('div');
    root.dataset.lyCertificatesRoot = '1';

    if (config.eyebrow.visible !== false && config.eyebrow.text) {
      const eyebrow = document.createElement('span');
      eyebrow.dataset.lyCertificatesEyebrow = '1';
      eyebrow.textContent = config.eyebrow.text;
      root.appendChild(eyebrow);
    }

    const title = document.createElement('h2');
    title.dataset.lyCertificatesTitle = '1';
    appendHighlightedText(title, config.title, config.highlight);
    root.appendChild(title);

    if (config.subtitle.visible !== false && config.subtitle.text) {
      const subtitle = document.createElement('p');
      subtitle.dataset.lyCertificatesSubtitle = '1';
      subtitle.textContent = config.subtitle.text;
      root.appendChild(subtitle);
    }

    const stage = document.createElement('div');
    stage.dataset.lyCertificatesStage = '1';
    const frame = document.createElement('div');
    frame.dataset.lyCertificatesFrame = '1';
    frame.dataset.glow = config.premiumGlow !== false ? 'true' : 'false';
    ...
    image.style.objectFit = config.imageFit;
    ...
    const updateImage = nextIndex => {
      ...
      section.dataset.lyCertificateIndex = String(index);
      image.src = images[index].src;
      image.alt = images[index].alt || `Certificado ${index + 1}`;
      const counter = root.querySelector('[data-ly-certificates-counter]');
      if (counter) counter.textContent = `${index + 1} de ${images.length}`;
    };

    const carousel = config.displayMode === 'carousel' && images.length > 1;
    if (carousel && config.showArrows !== false) {
      stage.append(
        makeButton('prev', () => updateImage(index - 1)),
        makeButton('next', () => updateImage(index + 1))
      );
    }
    root.appendChild(stage);

    if (carousel && config.showCounter !== false) {
      const counter = document.createElement('span');
      counter.dataset.lyCertificatesCounter = '1';
    ...
    if (config.benefits.visible !== false && config.benefits.items.length) {
      ...
      benefits.dataset.lyCertificatesBenefits = '1';
      config.benefits.items.forEach(text => {
        const item = document.createElement('div');
        item.dataset.lyCertificateBenefit = '1';
    ...
    if (config.final.visible !== false && (config.final.title || config.final.text)) {
      ...
      final.dataset.lyCertificatesFinal = '1';
      if (config.final.title) {
        const heading = document.createElement('h3');
        heading.textContent = config.final.title;
        final.appendChild(heading);
      }
      if (config.final.text) {
        const text = document.createElement('p');
        text.textContent = config.final.text;
  ...
  function apply(data) {
    const section = document.querySelector('section[data-edit-id="section-006"]');
    if (!section) return;
    const config = normalize(data?.site?.certificates);
